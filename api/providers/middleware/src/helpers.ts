...
export function copyGroupIdFromContextMiddlewares(groups = ['territory_id', 'operator_id'], targetPathPrefix: string = null, preserve = false): ListOfParametredMiddlewares<CopyFromContextMiddlewareParams> {
...
export function groupPermissionMiddlewares(groups: { user?: string, territory?: string, operator?: string, registry?: string }, targetPathPrefix: string = null): ListOfParametredMiddlewares<HasPermissionByScopeMiddlewareParams>  {
    const middlewareParameters = [];
    
    const { user: userPermission, territory: territoryPermission, operator: operatorPermission, registry: registryPermission } = groups;

    if (userPermission) {
        middlewareParameters.push([
            userPermission,
            'call.user._id',
            buildPathWithPrefix('_id', targetPathPrefix),
        ]);
    }
    ...
    return [
        hasPermissionByScopeMiddleware(registryPermission, ...middlewareParameters),
    ];
}

export function copyGroupIdAndApplyGroupPermissionMiddlewares(groups: { user?: string, territory?: string, operator?: string, registry?: string }, targetPathPrefix: string = null): ListOfParametredMiddlewares {
...
export function internalOnlyMiddlewares(...services: string[]): ListOfParametredMiddlewares<(ChannelServiceWhitelistMiddlewareParams | ChannelServiceWhitelistMiddlewareParams)> {
