...
test('[getBucketUrl] no var, no prefix', (t) => {
  delete process.env.AWS_BUCKET_PREFIX;
  delete process.env.AWS_BUCKET_EXPORT_URL;
  t.is(getBucketEndpoint('https://s3.test', BucketName.Export), 'https://export.s3.test');
});

test('[getBucketUrl] no var, with prefix', (t) => {
  process.env.AWS_BUCKET_PREFIX = 'url-prefix';
  delete process.env.AWS_BUCKET_EXPORT_URL;
  t.is(getBucketEndpoint('https://s3.test', BucketName.Export), 'https://url-prefix-export.s3.test');
});

test('[getBucketUrl] var override, no prefix', (t) => {
  delete process.env.AWS_BUCKET_PREFIX;
  process.env.AWS_BUCKET_EXPORT_URL = 'https://override-export.s3.test';
