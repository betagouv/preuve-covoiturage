...
import { phoneCustomKeyword } from './phoneCustomKeyword';

const { assert } = chai;

describe('phone number validation', () => {
  const yep = (s: string) => assert(phoneCustomKeyword.definition(s));
  const nope = (s: string) => assert(!phoneCustomKeyword.definition(s));
