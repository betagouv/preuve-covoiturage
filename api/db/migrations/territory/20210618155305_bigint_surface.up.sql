...
ALTER TABLE territory.territories ALTER COLUMN surface TYPE bigint USING surface::bigint;
...
CREATE VIEW trip.list_view AS (
  ...
  -- THIS IS FOR AUTH AND SEARCH ONLY --
    ...
    trunc(
      ...
      CASE WHEN (
        tts.surface > 0 AND 
        (tts.population::float / (tts.surface::float / 1000000)) > 40
      )
      THEN 3 
      ELSE 2
      END
    ...
    trunc(
      ...
      CASE WHEN (
        tte.surface > 0 AND 
        (tte.population::float / (tte.surface::float / 1000000)) > 40
      )
      THEN 3 
      ELSE 2
      END
