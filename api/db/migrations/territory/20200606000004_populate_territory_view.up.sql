...
-- fast
CREATE UNLOGGED TABLE mig.root AS
  SELECT
    t._id,
...
-- fast
CREATE UNLOGGED TABLE mig.codes AS
  SELECT
    territory_id,
...
-- fast
CREATE UNLOGGED TABLE mig.input AS
  SELECT
    r._id,
    r.parents,
    r.children,
...
-- slow
CREATE UNLOGGED TABLE mig.complete_parent AS
...
-- slow
CREATE UNLOGGED TABLE mig.complete_children AS
...
-- fast
CREATE UNLOGGED TABLE mig.complete AS
  SELECT
    cc._id,
    cc.children,
    cc.insee,
...
--slow
CREATE UNLOGGED TABLE mig.agg AS
  SELECT
    c._id,
...
-- fast
CREATE UNLOGGED TABLE mig.bc_array AS
  SELECT
