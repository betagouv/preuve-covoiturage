...
FROM territory.territory_codes tc GROUP BY tc.territory_id
  )
  ,input AS (
    SELECT 
      r._id,
  ...
  INSERT INTO territory.territories_view (_id,active,activable,level,parent,children, ancestors,descendants,insee,postcode,codedep,breadcrumb)
    SELECT
        a._id::int,
        t.active::boolean,
        t.activable::boolean,
