CREATE MATERIALIZED VIEW trip.list AS (
  SELECT
    ...
    (CASE WHEN cp.distance <> null THEN cp.distance ELSE (cp.meta::json->'calc_distance')::text::int END) as distance
  ...
  JOIN territory.insee as tie ON cp.end_insee = cie._id
);
