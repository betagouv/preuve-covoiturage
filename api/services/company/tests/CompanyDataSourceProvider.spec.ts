...
describe('Company data source provider', () => {
  ...
  it('should fetch from data source with a siret id', async () => {
    ...
    expect(data.nic)
      .to.be.a('string')
      .that.eq('00010');
    expect(data.legal_name)
      .to.be.a('string')
      .that.eq('SECRETARIAT GENERAL DU GOUVERNEMENT');
    expect(data.company_naf_code)
      .to.be.a('string')
      .that.eq('8411Z');
    expect(data.establishment_naf_code)
      .to.be.a('string')
      .that.eq('8411Z');
    expect(data.legal_nature_code)
      .to.be.a('string')
      .that.eq('7120');
    expect(data.legal_nature_label)
      .to.be.a('string')
      .that.eq("Service central d'un ministère");
    expect(data.nonprofit_code).to.be.null;
    expect(data.intra_vat).to.be.null; // FIXME
    expect(data.address)
      .to.be.a('string')
      .that.eq('57 Rue de Varenne 75007 Paris');
    expect(data.lon)
      .to.be.a('number')
      .that.eq(2.320884);
    expect(data.lat)
