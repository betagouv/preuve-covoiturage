...
export class ExportAction extends Action {
  ...
  private castOrGetDefaultDates(params: ParamsInterface): { start_date: Date; end_date: Date } {
    // use the local times
    const start_date_lc = get(params, 'query.date.start');
    const end_date_lc = get(params, 'query.date.end');

    // having both
    if (start_date_lc && end_date_lc) {
      return { start_date: start_date_lc, end_date: end_date_lc };
    }

    // make a 1 month date range from start_date
    if (start_date_lc && !end_date_lc) {
      return { start_date: start_date_lc, end_date: addMonths(start_date_lc, 1) };
    }

    // make a 1 month date range from end_date
    if (!start_date_lc && end_date_lc) {
      return { start_date: subMonths(end_date_lc, 1), end_date: end_date_lc };
    }
