...
export class MetadataStore implements MetadataStoreInterface {
  ...
  constructor(
    protected readonly repository?: MetadataRepositoryProviderInterfaceResolverV2,
  ) {}
  ...
  async load(registry: MetadataRegistryInterface): Promise<MetadataAccessorInterface> {
    ...
    return MetadataAccessor.import(registry.datetime, keys.map(k => this.cache.get(k)).reduce((m,i) => { m.set(i.uuid, i); return m;}, new Map()));
  }
  ...
  async store(lifetime: MetadataLifetime): Promise<Array<SerializedMetaInterface>> {
    const dataToSave: Array<SerializedMetaInterface> = new Array();
