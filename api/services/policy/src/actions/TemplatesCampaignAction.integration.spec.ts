...
import { handlerConfig, ParamsInterface, ResultInterface } from '../shared/policy/templates.contract';
...
test.skip(
  ...
  (response: ResultInterface, t: ExecutionContext<TestContext>) => {
    ...
    t.is(response.filter((c) => c.status !== 'template').length, 0);
    const policy = response.find((c) => c._id === t.context.policy_id);
    t.is(policy.name, fakeCampaign.name);
  },
  mockContext(['common.policy.list.templates']),
);
