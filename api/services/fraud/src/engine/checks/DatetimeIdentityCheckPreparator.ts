    ...
    const query = {
      text: `
        ...
        data AS (
          SELECT
            i.duration,
            EXTRACT(second FROM (cc.datetime - i.start_datetime)::interval) as start_interval,
            EXTRACT(second FROM ((cc.datetime + (cc.duration || ' seconds')::interval) - i.end_datetime)::interval) as end_interval
          FROM carpool.carpools as cc
          JOIN carpool.identities as id
            ON cc.identity_id = id._id
          JOIN input as i
