...
export class PopulateGeoCentroid extends AbstractDatatreatment {
  ...
  readonly sql = `
    ...
    SELECT year, 'com', arr, l_arr, centroid
    FROM ${this.targetTableWithSchema}
    WHERE com IS NOT NULL

    UNION

    SELECT year, 'epci', epci, l_epci, ST_PointOnSurface(st_union(geom_simple))
    FROM ${this.targetTableWithSchema}
    ...
    SELECT year, 'aom', aom, l_aom, ST_PointOnSurface(st_union(geom_simple))
    FROM ${this.targetTableWithSchema}
    ...
    SELECT year, 'dep', dep, l_dep, ST_PointOnSurface(st_union(geom_simple))
    FROM ${this.targetTableWithSchema}
    ...
    SELECT year, 'reg', reg, l_reg, ST_PointOnSurface(st_union(geom_simple))
    FROM ${this.targetTableWithSchema}
    ...
    SELECT distinct on (year,country) year, 'country', country, l_country, ST_PointOnSurface(st_union(geom_simple))
    FROM ${this.targetTableWithSchema}
    WHERE country <> 'XXXXX'
    ...
    SELECT year, 'country', country, l_country, centroid
    FROM ${this.targetTableWithSchema}
    WHERE arr = '75056'
