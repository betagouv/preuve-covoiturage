export class DependencyTree {
  nodes: Map<
    symbol,
    { data: any; require: Set<symbol>; requiredBy: Set<symbol> }
  > = new Map();
  ...
  protected getOrderedNodes(
    initialMet: Set<symbol> = new Set(),
    initialUnmet?: Set<symbol>,
  ): any {
    ...
    for (const nodeId of unmet) {
      const deps = this.nodes.get(nodeId)?.require || new Set();
      // list unmets dependencies
      const unmetDeps = new Set([...deps].filter((x) => !met.has(x)));

      // look for missing deps
      const unresolvableDependencies = new Set(
        [...unmetDeps].filter((x) => !unmet.has(x)),
      );
    ...
    if (
      requested.size > 0 &&
      new Set([...stillUnmet].filter((x) => !requested.has(x))).size === 0
    ) {
  ...
  protected inverseRelations() {
    ...
    for (const [nodeId, { requiredBy }] of nodes) {
      for (const req of requiredBy) {
        ...
        requirement && requirement.require.add(nodeId);
      }
    }
  }
}
