...
it("DataWorkBookWriter: should stream data to a workbook file", async (t) => {
  ...
  assertObjectMatch<excel.Row["values"], excel.Row["values"]>(
    sortRowValues(
      workbook.getWorksheet(dataWorkBookWriter.WORKSHEET_NAME).getRow(1).values,
    ),
    [undefined, ...Object.keys(exportTripInterface)].sort(),
  );
  assertEquals(
    workbook.getWorksheet(dataWorkBookWriter.WORKSHEET_NAME).getRow(2).values
      .length,
    Object.keys(exportTripInterface).length + 1,
  );
  assertEquals(
    workbook.getWorksheet(dataWorkBookWriter.WORKSHEET_NAME).getRow(2).getCell(
      1,
    ).value,
