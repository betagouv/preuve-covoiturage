...
beforeEach((t) => {
  ...
  t.context.userNotificationProvider = new UserNotificationProvider(
    null,
    null,
    null,
  );
  ...
  t.context.userNotificationProviderStub = sinon.stub(
    t.context.userNotificationProvider,
    "simulationEmail",
  );
...
it("SimulatePolicyformAction: should fail and return geo error if error in SimulateOnPastByGeoAction", async (t) => {
  // Arrange
  t.context.kernelInterfaceResolverStub!.throws(
...
it("SimulatePolicyformAction: should call simulation for 1, 3 and 6 months period", async (t) => {
  ...
  sinon.assert.calledOnceWithExactly(
    t.context.userNotificationProviderStub,
    params,
    {
