...
export function normalizeExport(
  ...
  timeZone: string,
): FlattenTripInterface {
  const { data, driver_incentive_raw, passenger_incentive_raw } = normalize(
    src,
    timeZone,
  );
...
export function normalizeOpendata(
  src: ExportTripInterface,
  timeZone: string,
): FlattenTripInterface {
  const { data, driver_incentive_raw, passenger_incentive_raw } = normalize(
  ...
  data.has_incentive = driver_incentive_raw.length > 0 ||
...
function normalize(
  ...
  const data: FlattenTripInterface = {
    ...
    journey_start_datetime: datetz.format(jsd, "yyyy-MM-dd'T'HH:mm:ssXXX", {
      timeZone,
    }),
    journey_start_date: datetz.format(jsd, "yyyy-MM-dd", { timeZone }),
    journey_start_time: datetz.format(jsd, "HH:mm:ss", { timeZone }),

    journey_end_datetime: datetz.format(jed, "yyyy-MM-dd'T'HH:mm:ssXXX", {
      timeZone,
    }),
    ...
    journey_duration_calculated: Math.round(
      src.journey_duration_calculated / 60,
    ),
