...
export class BuildResourceDescription {
  ...
  async call(
    ...
    const total_truncated: string =
      await (await this.tripRepository.searchCount(tripSearchQueryParam)).count;
    const start_deleted: number = excludedTerritories
      .filter((e) => e.start_geo_code)
      .reduce(
        (count, value) => count + value.aggregated_trips_journeys.length,
        0,
      );
  ...
  private async getTotal(
    ...
    return (await this.tripRepository.searchCount(tripSearchQueryParamCopy))
      .count;
  }
  ...
  private build(
    ...
    return `# Spécificités jeu de données ${
      startDatePlus6Days.toLocaleString("fr-FR", {
        month: "long",
      })
