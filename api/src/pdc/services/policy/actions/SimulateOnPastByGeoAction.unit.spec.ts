...
beforeEach((t) => {
  ...
  t.context.tripRepositoryResolverStub = sinon.stub(
    t.context.tripRepository,
    "findTripByGeo",
  );

  t.context.todayMinusSizMonthes = new Date();
  t.context.todayMinusSizMonthes.setMonth(
    t.context.todayMinusSizMonthes.getMonth() - 6,
  );
});
...
it("SimulateOnPastByGeoAction: should process trip with default time frame", async (t) => {
  ...
  t.context.tripRepositoryResolverStub!.callsFake(function* fake() {
    const carpool: Partial<CarpoolInterface> = {
      ...
      datetime: faker.date.between({
        from: t.context.todayMinusSizMonthes,
        to: new Date(),
      }),
      seats: 1,
      distance: 25000,
  ...
  // Act
  const result: ResultInterface = await t.context.simulateOnPasGeoAction!
    .handle({
