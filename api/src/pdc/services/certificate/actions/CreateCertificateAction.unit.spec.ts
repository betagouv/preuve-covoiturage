...
interface Context {
  ...
  certificateRepositoryCreateStub: SinonStub<
    [params: CertificateBaseInterface]
  >;
  kernelCallStub: SinonStub<
...
beforeEach((t) => {
  ...
  t.context.kernelCallStub
    .onCall(1)
    .resolves({
      uuid: t.context.OPERATOR_UUID,
      name: t.context.OPERATOR_NAME,
      support: t.context.OPERATOR_SUPPORT,
    });
});
...
it("CreateCertificateAction: should generate certificate payload", async (t) => {
  ...
  // Act
  const result: WithHttpStatus<ResultInterface> = await t.context
    .createCertificateAction.handle(params, null);
  ...
  sinon.assert.calledOnceWithExactly(
    t.context.certificateRepositoryCreateStub,
