...
import sql, { raw, empty } from '../helpers/sql';
  ...
  public async save(data: InsertableCarpoolRequest, client?: PoolClient): Promise<WrittenCarpoolRequest> {
    ...
    const sqlQuery = sql`
      INSERT INTO ${raw(this.table)} (
        carpool_id, operator_id, operator_journey_id, payload, api_version, ${raw('created_at' in data ? 'created_at, ': '')} cancel_code, cancel_message
      ) VALUES (
        ${data.carpool_id},
        ${data.operator_id},
        ...
        ${data.api_version},
        ${'created_at' in data ? `${data.created_at},`: empty}
