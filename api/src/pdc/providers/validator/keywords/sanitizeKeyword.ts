...
import filterXSS, { IFilterXSSOptions } from "dep:xss";
...
export const sanitizeKeyword: KeywordDefinition = {
  ...
  compile(opt: SanitizeInterface) {
    ...
    const options: IFilterXSSOptions = typeof opt === "object" ? { ...opt } : {
    ...
    return (data: string, dataCtx: any): boolean => {
      ...
      if ("parentData" in dataCtx && "parentDataProperty" in dataCtx) {
        dataCtx.parentData[dataCtx.parentDataProperty] = filterXSS(
          data,
          options,
        );
        return true;
