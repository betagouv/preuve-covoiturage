...
import { format, subDays } from "dep:date-fns";
import { fr } from "dep:date-fns-locale";
...
export class DataGouvMetadataProvider {
  ...
  description(params: DataGouvDescriptionParams): string {
    const start_date_fr = format(params.start_at, "PP", { locale: fr });
    const end_date_fr = format(subDays(params.end_at, 1), "PP", { locale: fr });
