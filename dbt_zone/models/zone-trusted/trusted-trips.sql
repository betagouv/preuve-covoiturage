{{ 
    config(
      ...
      indexes = [
        ...
        {'columns': ['operator_journey_id']}
      ]
    )
}}

WITH carpools_data AS (
    ...
    SELECT
        ...
        cg.start_geo_code,
        cg.end_geo_code,
        cg.updated_at      AS geo_updated_at,
        cg.errors          AS geo_errors,
        ...
        -- Statuts finaux
        (cs.acquisition_status IN ('processed','failed','canceled','expired','terms_violation_error'))::boolean AS final_status,
        (cs.acquisition_status = 'processed'
         AND cs.anomaly_status = 'passed'
         AND cs.fraud_status = 'passed')::boolean AS valid_status
    ...
    LEFT JOIN {{ source('operator', 'operators') }} AS oo ON cc.operator_id = oo._id
