...
export default function CampaignsTable(props: {title:string, territoryId:number | null}) {
  ...
  const { data } = useApi<Record<string, string | number>[]>(props.territoryId ? `${url}?territory_id=${props.territoryId}` : url);
  ...
  const dataTable = data?.map((d,i) => [
    ...
    <Button key={i} size="small" onClick={()=>setCampaignId(d.id as number)}>Détails</Button>
      ...
      {campaignId && currentCampaign &&
        <>
          ...
          <div className={fr.cx('fr-callout')}>
            ...
            <div><b>Nom de la campagne :</b> {currentCampaign.name}</div>
            <div><b>Durée de la campagne :</b> Du {new Date(currentCampaign.start_date).toLocaleDateString()} au {new Date(currentCampaign.end_date).toLocaleDateString()}</div>
            <div><b>Estimation de la consommation du budget* :</b> {`${currentCampaign.incentive_sum.toLocaleString()} € sur ${currentCampaign.max_amount.toLocaleString()} €`}</div>
            <i>* A noter que le budget est le montant dédié aux incitations uniquement et qu’il s’agit ici d’une estimation de la consommation en quasi temps réel. </i>
          </div>
          <ApdfTable title='Fichiers d’appels de fonds recalculés par covoiturage.beta.gouv' campaignId={campaignId} />
        </>
      }
    </>    
  );
