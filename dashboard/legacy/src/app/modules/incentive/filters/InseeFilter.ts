...
export class InseeFilter {
  ...
  public static toString(data): string {
    ...
    if (data.blackList.or.start.length > 0) {
      text += `&nbsp; - les codes INSEEs de départ ne sont pas parmi : ${data.blackList.or.start
        .sort()
        .join(', ')} <br>`;
    }
    ...
    if (data.whiteList.and.start.length > 0 && data.whiteList.and.end.length > 0) {
      ...
      text += `&nbsp; &nbsp; ET les codes INSEEs d'arrivée sont parmi : ${data.whiteList.and.end
        .sort()
        .join(', ')} <br>`;
    }
    if (data.blackList.and.start.length > 0 && data.blackList.and.end.length > 0) {
      text += `&nbsp; - les codes INSEEs de départ ne sont pas parmi : ${data.blackList.and.start
        .sort()
        .join(', ')} <br>`;
      text += `&nbsp; &nbsp; ET les codes INSEEs d'arrivée ne sont pas parmi : ${data.blackList.and.end
        .sort()
        .join(', ')}`;
    }
    return text;
