...
export class UserDropdownComponent implements OnInit {
  ...
  set users(users: User[]) {
    if (users) {
      this.formattedUsers = users.map((user: User) => {
        // tslint:disable-line ter-arrow-body-style
        return {
          key: user._id,
  ...
  public ngOnInit() {
    if (this.user && this.user.value) {
      this.formattedUser = {
        key: this.user.value._id,
        value: `${this.user.value.firstname} ${this.user.value.lastname}`,
      };
    }
  }
