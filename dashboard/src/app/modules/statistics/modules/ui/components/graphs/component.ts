...
export class StatisticsGraphsComponent implements OnInit {
  ...
  private loadGraphs() {
    const stats = STAT_STYLE.details.filter(
  ...
  private loadValidatedClassesGraph(config) {
    ...
    datas['A'] = this.statisticsService.getDataFromPathString(
      'classes.a.total',
      this.apiData,
    );
    datas['B'] = this.statisticsService.getDataFromPathString(
      'classes.b.total',
      this.apiData,
    );
    datas['C'] = this.statisticsService.getDataFromPathString(
      'classes.c.total',
      this.apiData,
    );
  ...
  private getLabels(values: [Statistic]) {
    return values.map(
      (value: Statistic) =>
        new Date(value._id.year, value._id.month - 1, value._id.day),
    );
  }
