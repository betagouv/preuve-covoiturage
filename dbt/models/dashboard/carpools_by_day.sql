...
SELECT
  ...
  operator_id,
  operator_name,
  count(distinct carpool_id) AS journeys,
  count(distinct carpool_id) filter (where coalesce(incentive_amount,0) > 0) AS incented_journeys,
