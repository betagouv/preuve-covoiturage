{{ config(
    ...
    post_hook=[
      "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'occupation_by_month_pkey') THEN ALTER TABLE {{ this }} ADD CONSTRAINT occupation_by_month_pkey PRIMARY KEY (year, month, code, type, direction); END IF; END $$;"
      "CREATE INDEX IF NOT EXISTS occupation_by_month_idx ON {{ this }} using btree(year, month, code, type, direction)",
    ]
  )
}}
