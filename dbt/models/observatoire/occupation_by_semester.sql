{{ config(
    ...
    post_hook=[
      "DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'occupation_by_semester_pkey') THEN ALTER TABLE {{ this }} ADD CONSTRAINT occupation_by_semester_pkey PRIMARY KEY (year, semester, code, type, direction); END IF; END $$;"
      "CREATE INDEX IF NOT EXISTS occupation_by_semester_idx ON {{ this }} using btree(year, semester, code, type, direction)",
    ]
  )
}}
